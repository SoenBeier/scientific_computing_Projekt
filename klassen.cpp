...
// ###### Dynamic floor field DZur Uni Potsdam(Brandenburg):
    ...
    void set_panic_par(vector <person> &persvec, int i, int iteration)
    {
        ...
        if (persvec[i].iterat_val+2 <= persvec[i].iteration && persvec[i].had_a_conflict==false && persvec[i].evacuated==false && persvec[i].panic_par >= 0)
        {
            persvec[i].panic_par--;
            if(panic_par < panik_schwelle && r==255){
            r=0;
            g=250;
            b=0;
            }
